...
class Camera:
    ...
    def command_camera(self, command: str) -> None:
        ...
        # Build image name
        image_name = f"{self.collection_name}_{self.image_index:05d}{self.image_type}"
        # Command Camera
        # start = time.time()
        system(f'\"{self.control_cmd_location}\" /filename {self.save_folder}{image_name} {command}')
        # end = time.time()
        # print(end-start)
        # 약 6~7초 소요 이게 제일 오래걸림
...
if __name__ == '__main__':
    camera = Camera(control_cmd_location='C:\Program Files (x86)\digiCamControl\CameraControlCmd.exe', 
                    collection_name="DSC", 
                    save_folder='./digicam_img/'
                    )
    # start = time.time()
